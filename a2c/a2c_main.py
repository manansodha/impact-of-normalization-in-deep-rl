...
def choose_update(agent: Agent, method: str) -> float:
    if method == "vanilla":
        return agent.vanilla_a2c_update()
    if method == "grad_clip":
...
def plot_results(
    ...
    ax2.set_ylabel("Average A2C Loss")
    ax2.set_xlabel("A2C Update")
    ax2.legend()
    ...
    ax5.set_xlabel("A2C Update")
    ax5.legend()

    fig.suptitle(f"A2C Training Stability - {method}", fontsize=16, fontweight="bold")
    plt.show()
...
def train_seed(env: gym.Env, dummy_obs_space: Box, config: TrainConfig, seed: int) -> Dict[str, List[float]]:
    ...
    return {
        ...
        "policy_loss": agent.policy_loss_history,
        "value_loss": agent.value_loss_history,
        "episodes": [episode],
...
def main() -> int:
    ...
    try:
        ...
        policy_df = pd.DataFrame(
            {seed: pd.Series(res["policy_loss"]).reindex(range(1, config.batches + 1)) for seed, res in
             seed_results.items()}
        )
        value_df = pd.DataFrame(
            {seed: pd.Series(res["value_loss"]).reindex(range(1, config.batches + 1)) for seed, res in
             seed_results.items()}
        )
        ...
        loss_df.to_csv(f"{config.method}_a2c_loss_history.csv")
        reward_df.to_csv(f"{config.method}_a2c_reward_history.csv")
